...
// --- Routed sensor multiplexer select lines (one of 8 screw terminals) ---
// Used here only to route the routed-sensor rail through to slot 2, where the
// SMT100 is plugged in. Matches the wiring used in Test_Read_SMT100_NewBoard.ino.
#define routedSensorS0 25  // LSB
#define routedSensorS1 14
#define routedSensorS2 15  // MSB

// --- SMT100 (Truebner) on routed-sensor slot 2, UART2 over the multiplexed lines ---
// Screw terminal pinout: 3v3 | 33 | 32 | GND
//   ESP32 RX = GPIO32 -- yellow wire from sensor
//   ESP32 TX = GPIO33 -- green  wire to   sensor
// Power is gated by the mux (S0/S1/S2) plus loraRoutedSensorEnPin.
#define SMT100_SENSOR_SLOT 2
#define SMT100_RX_PIN 32   // ESP32 RX  (yellow from sensor)
#define SMT100_TX_PIN 33   // ESP32 TX  (green to sensor)
HardwareSerial SMTSerial(2);
...
String smt100SendCommand(const String &cmd, unsigned long timeoutMs);
float smt100ParseValue(const String &resp);
void readAndPackSMT100(uint8_t *buffer, size_t *payloadLength, size_t max);
...
// LSB
static const u1_t PROGMEM DEVEUI[8] = { 0x0E, 0x7C, 0x07, 0xD0, 0x7E, 0xD5, 0xB3, 0x70 };
void os_getDevEui(u1_t *buf) { memcpy_P(buf, DEVEUI, 8); }

// MSB
static const u1_t PROGMEM APPKEY[16] = { 0xDB, 0xB3, 0x84, 0xDE, 0x6C, 0xF8, 0xAD, 0xA7, 0x42, 0x58, 0xED, 0xE4, 0x80, 0x14, 0xD9, 0x0B };
...
// ===== Routed sensor multiplexer + SMT100 (Truebner) over UART2 =====

/**
 * Selects a routed sensor slot (0..7) on the multiplexer.
 * Logic matches Test_Read_SMT100_NewBoard.ino exactly.
 */
void selectSensor(uint8_t sensor) {
...
/**
 * Send a single command to the SMT100 and return the trimmed response,
 * or "TIMEOUT" if no line is received within timeoutMs.
 */
String smt100SendCommand(const String &cmd, unsigned long timeoutMs) {
  while (SMTSerial.available()) SMTSerial.read();
...
/**
 * Parse the numeric value out of an SMT100 response such as
 *   "GetTemperature_+22.34"  or  "GetWaterContent_+12.50"
 * Returns NAN if the response is "TIMEOUT" or no number can be parsed.
 */
float smt100ParseValue(const String &resp) {
  if (resp == "TIMEOUT" || resp.length() == 0) return NAN;

  // Find the first character that can start a number (+, -, digit, or '.')
  int idx = -1;
  ...
  String numStr = resp.substring(idx);
  return numStr.toFloat();
}

/**
 * Reads SMT100 sensor values and packs them into the payload buffer.
 *   - Temperature (i16, °C × 100)   (signed)
 *   - Water content (u16, % × 100)  (unsigned)
 * On failure, zeros are written so the payload length stays constant.
 *
 * Sequence mirrors Test_Read_SMT100_NewBoard.ino (which is known good):
 *   1. select mux slot 2 BEFORE powering the rail
 *   2. enable the routed-sensor rail
 *   3. wait 1 s for the SMT100 to boot
 *   4. open UART2 on the routed pins (GPIO32 RX, GPIO33 TX)
 *   5. send GetTemperature / GetWaterContent
 *   6. close UART, return mux to slot 0
 */
void readAndPackSMT100(uint8_t *buffer, size_t *payloadLength, size_t max) {
  Serial.println("==============================================");

  // 1) Select the SMT100 slot BEFORE powering the rail, so the right
  //    terminal receives 3v3 when we enable the rail.
  selectSensor(SMT100_SENSOR_SLOT);

  // 2) Power the routed-sensor rail
  digitalWrite(loraRoutedSensorEnPin, HIGH);

  // 3) SMT100 needs ~1 s to boot after power-up
  delay(1000);

  // 4) Start UART2 on the screw-terminal pins
  SMTSerial.begin(9600, SERIAL_8N1, SMT100_RX_PIN, SMT100_TX_PIN);

  // 5) Query the sensor
  //    GetTemperature usually answers within ~1 s.
  //    GetWaterContent can take a few seconds (TDR settle), so allow 5 s.
  String tResp = smt100SendCommand("GetTemperature!000000", 1000);
  delay(200);
  String wResp = smt100SendCommand("GetWaterContent!000000", 5000);
  delay(200);

  Serial.print("SMT100 Temp raw : ");
  Serial.println(tResp);
  Serial.print("SMT100 Water raw: ");
  Serial.println(wResp);

  float tC = smt100ParseValue(tResp);
  float wPct = smt100ParseValue(wResp);

  int16_t  tPacked = 0;
  uint16_t wPacked = 0;

  if (!isnan(tC)) {
    long v = lroundf(tC * 100.0F);
    if (v < -32768) v = -32768;
    if (v >  32767) v =  32767;
    tPacked = (int16_t)v;
  }
  if (!isnan(wPct)) {
    long v = lroundf(wPct * 100.0F);
    if (v < 0)     v = 0;
    if (v > 65535) v = 65535;
    wPacked = (uint16_t)v;
  }

  put_i16(buffer, payloadLength, max, tPacked);
  put_u16(buffer, payloadLength, max, wPacked);

  Serial.print("SMT100 packed -> T=");
  Serial.print(tPacked / 100.0F);
  Serial.print("C  W=");
  Serial.print(wPacked / 100.0F);
  Serial.println("%");

  // 6) Release UART2 and return mux to slot 0.
  SMTSerial.end();
  selectSensor(0);
  // loraRoutedSensorEnPin is left HIGH (matches the BME280/SHT31 readers).
  // It is implicitly de-energized when the ESP enters deep sleep after TX.
...
void refreshSensorData() {
  ...
  // SMT100 (Truebner) on routed-sensor slot 2.
  // Always called -- if the sensor isn't connected, the reader packs zeros.
  readAndPackSMT100(buffer, &payloadLength, sizeof(buffer));
...
void setup() {
  ...
  // Multiplexer select lines for the routed sensors (SMT100 is on slot 2)
  ...
  os_init();
  LMIC_reset();
  // ESP32's RTC crystal is too imprecise for LMIC's default RX-window timing.
  // Without this line, the JoinAccept arrives but the radio's RX window is
  // already closed, and you get "EV_JOIN_TXCOMPLETE: no JoinAccept" even
  // though the TTN console shows the JoinAccept being sent.
  // 1% is the recommended starting value; increase to 10% if joins still fail.
...
void setSleepTime() {
  ...
  if (CellLvlPercent > 90) {
    TIME_TO_SLEEP = 300ULL;       // 5 minutes
  } else if (CellLvlPercent > 70) {
    TIME_TO_SLEEP = 1200ULL;      // 10 minutes
